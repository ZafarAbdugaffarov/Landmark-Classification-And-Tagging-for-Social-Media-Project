...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        train_loss, train_accuracy = train_one_epoch(data_loaders["train"], model, optimizer, loss)
        valid_loss, valid_accuracy = valid_one_epoch(data_loaders["valid"], model, loss)

        # Print training/validation statistics
        print(
            f"Epoch: {epoch} \tTraining Loss: {train_loss:.6f} \tTraining Accuracy: {train_accuracy:.6f} \tValidation Loss: {valid_loss:.6f} \tValidation Accuracy: {valid_accuracy:.6f}"
        )
...
def one_epoch_test(test_dataloader, model, loss):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            test_loss += ((1 / (batch_idx + 1)) * (loss_value.data.item() - test_loss))
